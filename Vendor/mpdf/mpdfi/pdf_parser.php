...
class pdf_parser {
	...
	function pdf_parser($filename) {
     ...
     $this->f = fopen($this->filename,'r',false,$context);//, "rb");
    ...
    try { 
	  $offset = $this->pdf_find_xref();
        if ($offset===false) {
            $this->success = false;
            $this->errormsg = sprintf("Cannot get offset %s !", $filename);
		return false;
	  }}catch (Exception $e) {
    ...
    function pdf_find_xref() {
      ...
      //fseek int really working properly from a S3 bucket so we will write to local tmp file
  
      fseek($this->f,0);
      $content = stream_get_contents($this->f); 
      $filename =  tempnam("" , "PDFMASH");
      $this->f = fopen($filename,"w");
      fwrite($this->f,$content);
      fclose($this->f);
      $this->f = fopen($filename,'r',false);
      $this->c = new pdf_context($this->f);
